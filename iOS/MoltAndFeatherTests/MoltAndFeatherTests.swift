import XCTest
@testable import MoltAndFeather

@MainActor
final class MoltAndFeatherTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(FeatherFind(species: "Test Entry"))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testAddedEntryAppearsFirst() {
        store.add(FeatherFind(species: "Newest"))
        XCTAssertEqual(store.entries.first?.species, "Newest")
    }

    func testDeleteRemovesEntry() {
        let entry = FeatherFind(species: "ToDelete")
        store.add(entry)
        store.delete(entry)
    ...
    func testCannotAddMoreAtFreeLimitWithoutPro() {
        store.entries = (0..<Store.freeTierLimit).map { _ in FeatherFind(species: "X") }
