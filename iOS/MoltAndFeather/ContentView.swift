...
struct ContentView: View {
    ...
    @State private var editingEntry: FeatherFind?
        ...
        NavigationStack {
            ZStack {
                ...
                if store.entries.isEmpty {
                    ContentUnavailableView("No Feather Find Entries Yet", systemImage: "square.and.pencil", description: Text("Tap + to add your first entry."))
...
struct EntryEditorView: View {
    ...
    let entry: FeatherFind?

    @State private var draft: FeatherFind
    @State private var draftDate: Date = Date()

    init(entry: FeatherFind?) {
        self.entry = entry
        _draft = State(initialValue: entry ?? FeatherFind())
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                Theme.background.ignoresSafeArea()
                Form {
                TextField("Species Guess", text: $draft.species)
